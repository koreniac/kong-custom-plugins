...
local jwt_decoder = require "kong.plugins.custom-jwt.jwt_parser"
...
local CustomJwtHandler = BasePlugin:extend()

CustomJwtHandler.PRIORITY = 1005
CustomJwtHandler.VERSION = "0.1.0"
...
function CustomJwtHandler:new()
  CustomJwtHandler.super.new(self, "custom-jwt")
end
...
local function do_authentication(conf)
  ...
  local jwt_uid = claims[conf.uid_claim_name]
  set_uid(jwt_uid)

  local scopes = claims[conf.scopes_claim_name]
  if not scopes then 
    return false, {status = 403, message = "No scope"}
  end

  local has_permission = false
  for _, v in ipairs(scopes) do
    if string.find(ngx.var.uri, v) then
      has_permission = true
      break
    end
  end
  ...
  if not has_permission then 
    return false, {status = 403, message = "Unavailable scope"}
  end
...
function CustomJwtHandler:access(conf)
  CustomJwtHandler.super.access(self)
